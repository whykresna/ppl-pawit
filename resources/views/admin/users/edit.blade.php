...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="lahan_id">{{ trans('cruds.user.fields.lahan') }}</label>
                <select class="form-control select2 {{ $errors->has('lahan') ? 'is-invalid' : '' }}" name="lahan_id" id="lahan_id" required>
                    @foreach($lahans as $id => $lahan)
                        <option value="{{ $id }}" {{ ($user->lahan ? $user->lahan->id : old('lahan_id')) == $id ? 'selected' : '' }}>{{ $lahan }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.lahan_helper') }}</span>
            </div>
            <div class="form-group">
