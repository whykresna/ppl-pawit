    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.penjualans.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tengkulak') ? 'is-invalid' : '' }}" name="tengkulak_id" id="tengkulak_id" required>
                    @foreach($tengkulaks as $id => $tengkulak)
                        <option value="{{ $id }}" {{ old('tengkulak_id') == $id ? 'selected' : '' }}>{{ $tengkulak }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_pengiriman') ? 'is-invalid' : '' }}" type="text" name="tanggal_pengiriman" id="tanggal_pengiriman" value="{{ old('tanggal_pengiriman') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_permintaan') ? 'is-invalid' : '' }}" type="number" name="jumlah_permintaan" id="jumlah_permintaan" value="{{ old('jumlah_permintaan') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('harga_penjualan') ? 'is-invalid' : '' }}" type="number" name="harga_penjualan" id="harga_penjualan" value="{{ old('harga_penjualan') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pajak_penjualan') ? 'is-invalid' : '' }}" type="number" name="pajak_penjualan" id="pajak_penjualan" value="{{ old('pajak_penjualan') }}" step="1" required>
