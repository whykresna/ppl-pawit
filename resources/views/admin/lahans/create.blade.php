...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.lahan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lahans.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="nama_lahan">{{ trans('cruds.lahan.fields.nama_lahan') }}</label>
                <input class="form-control {{ $errors->has('nama_lahan') ? 'is-invalid' : '' }}" type="text" name="nama_lahan" id="nama_lahan" value="{{ old('nama_lahan', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('luas_lahan') ? 'is-invalid' : '' }}" type="number" name="luas_lahan" id="luas_lahan" value="{{ old('luas_lahan') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_bibit') ? 'is-invalid' : '' }}" type="number" name="jumlah_bibit" id="jumlah_bibit" value="{{ old('jumlah_bibit') }}" step="1" required>
